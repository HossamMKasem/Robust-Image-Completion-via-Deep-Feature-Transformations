...
im=torch.zeros(cahnnel,S4/2,S4/2)
output_2=torch.zeros(testimg:size()[1],cahnnel,S4/2,S4/2)
label_temp=torch.zeros(cahnnel,S4,S4)
label_temp_scaled=torch.zeros(cahnnel,S4/2,S4/2)
Final_label=torch.zeros(label:size()[1],cahnnel,S4/2,S4/2)

for iii = 1,testimg:size(1) do
im2 = testimg[iii]
im = image.scale(im2, im2:size()[3] / 2, im2:size()[2] / 2, 'bicubic')
label_temp=label[iii]
label_temp_scaled=image.scale(label_temp, im2:size()[3] / 2, im2:size()[2] / 2, 'bicubic')
